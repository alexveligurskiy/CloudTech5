...
function enableButtons () {
	...
	btn = document.getElementById("input_greet_by_surname"); 
	btn.onclick=function(){greetBySurname();}; 

	// Update the button label now that the button is active 
	btn.value="Click me for a surname greating";
}
...
function greetBySurname (){
	
	var name = document.getElementById("name_field").value; 
	var surname = document.getElementById("surname_field").value;
	var period = document.getElementById("period_field").value; 
	
	var request = gapi.client.helloworldendpoints.greetBySurname({'name': name, 'surname':surname, 'period':period}); 
	request.execute(sayHelloCallback);
}
